...
"ubuntu")
		...
		. ${d}/_installUbuntu.sh
                ...
                ##/usr/bin/bash ${d}/_installUbuntu22.sh "$1"
		. ${d}/_installUbuntu22.sh
        else
...
#                echo ''
#                echo -e "Your system ${os_version} is not supported!"
#                exit 1
...
#                rpm -e --nodeps centos-repos  &>/dev/null
#                rpm -e --nodeps centos-linux-repos  &>/dev/null
#		rpm -Uv --replacepkgs http://31.42.190.125/repo/alma-brainy-repo-v3-9.0-2.el9.noarch.rpm  &>/dev/null
...
echo ""
echo "*******  INSTALL BRAINYCP CORE V3.02  *******"
echo ""
...
#. ${PWD_L}/_checkpkg.sh -u auto
	...
	#    exit 0
        ...
        if [ "${os_type}" == "rocky" ];then
            /bin/rm -f ${d}/_installCentos_t01v9Rocky.sh &>/dev/null
            wget -t 2  $ARRD/_installCentos_t01v9Rocky.sh &>/dev/null
        else
        ...
        if [ "${os_type}" == "rocky" ];then
. ${d}/_installCentos_t01v9Rocky.sh
        else
...
#rpm -Uv --replacepkgs http://46.175.147.85/repo/alma-brainy-repo-v3-9.0-2.el9.noarch.rpm  &>/dev/null
